...
class CopyWriterStep(KernelProcessStep[ProcessState]):
    ...
    @cl.step(name="Writer Agent", type="llm")
    ...
    async def generate_report(self, context: KernelProcessStepContext):
        ...
        match response.action:
            case "code":
                ...
                logger.debug("Code requested by agent: %s", response.content)
            ...
            case "search":
                ...
                logger.debug("Search requested by agent: %s", response.content)
                await context.emit_event(
            ...
            case "finished":
                ...
                logger.debug("Report content: %s", response.content)
            ...
            case "clarification":
                logger.debug("Clarification requested by agent: %s", response.content)
                await context.emit_event(
...
class SearchStep(KernelProcessStep[ProcessState]):
    ...
    @cl.step(name="Search Agent", type="llm")
...
class CodeGenerationStep(KernelProcessStep[ProcessState]):
    ...
    @cl.step(name="Coder Agent", type="llm")
...
class ReportEditorStep(KernelProcessStep[ProcessState]):
    ...
    @cl.step(name="Editor Agent", type="llm")
...
class UserStep(KernelProcessStep[ProcessState]):
    ...
    # @cl.step(name="User Approval", type="tool")
    ...
    async def wait_for_user_approval(self, context: KernelProcessStepContext):
        ...
        user_input = await cl.AskActionMessage(
            content=f"{self.state.report_content}\n\nDo you approve the report?",
            actions=[
                cl.Action(
                    name="yes",
                    payload={"value": "yes"},
                    label="Yes, approve the report",
                ),
                cl.Action(
                    name="no",
                    payload={"value": "no"},
                    label="No, I want to make changes",
                ),
            ],
            timeout=60,
            author="Editor Agent",
        ...
        user_input = await cl.AskUserMessage(
            content="Please provide feedback: ", timeout=60, author="Editor Agent"
        ).send()
        if user_input is None:
            print("No feedback provided, proceeding with the process.")
            await context.emit_event(process_event=self.Events.RequestEdits)
            return
        self.state.customer += (
            f" Please make changes to the report based on this feedback: {user_input}"
        )
        self.state.write_state()
        await context.emit_event(process_event=self.Events.RequestEdits)

    # @cl.step(name="User Clarification", type="tool")
    ...
    async def request_user_clarification(
        ...
        user_input = await cl.AskUserMessage(
            content=f"Please provide clarification for: {clarification}",
            timeout=60,
            author="Writer Agent",
        ).send()
        if not user_input:
            print("No clarification provided, proceeding with the process.")
...
class PDFGenStep(KernelProcessStep[ProcessState]):
    ...
    @cl.step(name="PDF Generation", type="tool")
...
async def set_starters():
    return [
        cl.Starter(
            label="SocGen",
            message="Societe Generale is a large bank based in France. ",
        ),
        cl.Starter(
            label="Microsoft",
            message="Microsoft is a leading technology company based in the USA.",
        ),
    ]
...
async def on_message(message: cl.Message):
    ...
    success = await run_process(customer=message.content)
    await cl.Message(
        content=success,
        author="Report Generation Process",
        elements=[
            cl.Pdf(name="Report", path=get_state_file_path().with_suffix(".pdf"))
        ],
    ).send()
